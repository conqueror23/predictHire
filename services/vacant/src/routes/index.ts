...
const getFilter = (params: string): Partial<VacantFilterParam> => {
...
app.post("/createDocument", async (req: Request, res: Response) => {
  try {
    const {title,description,expiredAt,companyId} = req.body;
    if (!title || !description || !expiredAt || !companyId) {
      res.send({
        status: 422,
    ...
    const data = await createDocument(MONGO_COLLECTION, {title,description,expiredAt,companyId});
    res.send({
      status: 200,
      message: data,
    });
...
app.put("/updateById", async (req: Request, res: Response) => {
  try {
    ...
    const { _id, ...content }:Partial<VacantFilterParam> = req.body;
    if (!_id) {
      res.send({
        status: 422,
